...
class GeneralTerms extends StatelessWidget {
  @override
...
이 약관은 마가렛(이하 "회사")에서 제공하는 "마가렛" 및 "마가렛"에서 제공하는 제반 서비스(이하 "서비스")에 접속과 사용자에 의해서 업로드 및 다운로드 되어 표시되는 모든 정보, 텍스트, 이미지 및 기타 자료를 이용하는 이용자(이하 "회원")와 서비스 이용에 관한 권리 및 의무와 책임사항, 기타 필요한 사항을 규정하는 것을 목적으로 합니다.
...
1. 서비스: 개인용 컴퓨터 (PC), TV, 휴대형 단말기, 전기통신설비 등 포함 각종 유무선 장치와 같이 구현되는 단말기와 상관없이 회원이 이용할 수 있는 "마가렛" 및 "마가렛" 관련 제반 서비스를 의미합니다.
...
1. 이용계약은 회원이 "마가렛" 서비스 및 제반 서비스에서 제공하는 회원가입 페이지에서 서비스 이용약관에 동의한 후 이용신청을 하고 신청한 내용에 대해서 회사가 승낙함으로써 체결됩니다.
...
19. 기타 "마가렛" 모바일 어플리케이션 또는 온라인 웹 페이지를 통하여 추가로 공지되는 사항에 위배되는 경우
...
4. 회사는 회원과 별도로 서면 계약을 체결하여 "마가렛" 서비스 및 제반 서비스의 브랜드 특성을 이용할 수 있는 명시적인 권리를 부여하지 아니하는 한, 회원에게 회사 또는 "마가렛" 서비스의 상호, 상표, 서비스표, 로고, 도메인 네임 및 기타 식별력 있는 브랜드 특성을 이용할 수 있는 권리를 부여하지 않습니다.

5. "마가렛" 및 "마가렛" 에서 제공하는 서비스는 계속 발전하고 있습니다. 회사가 제공하는 서비스의 형태와 기능, 디자인 등 필요한 경우 수시로 변경되거나, 중단될 수 있습니다. 회사는 이 경우 개별적인 변경에 대해서 회원에게 사전 통지하지 않습니다. 다만, 회원에게 불리한 것으로 판단되는 경우 공지 또는 전자우편으로 통하여 이를 공지합니다.
...
4. 회사가 본 약관 제9조의 위반 행위를 조사하는 과정에서 당해 회원 ID 및 도메인이 특정 위반행위에 직접적으로 관련되어 있는 경우 등 다른 회원의 권익 보호 및 서비스의 질서유지를 위해 불가피할 경우에는 해당 ID 및 도메인의 이용을 일시적으로 정지할 수 있습니다. 이에 대해 회원은 "마가렛" 서비스 홈페이지 또는 전자우편 등을 통해 이의신청을 할 수 있습니다.
...
3. 회사는 "마가렛" 를 기반으로 한 거래시스템만을 제공할 뿐이며, 거래 내용에 관한 모든 분쟁에 대해서는 해당 거래 당사자가 책임을 져야 합니다.
